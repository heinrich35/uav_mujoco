...
#
# Training progress analysis for G1 Standing IK **V2** (Isaac-G1-Standing-IK-V2-Direct-v0).
# Same wide layout as print_g1_standing_ik_progress_log.py; sidebar leads with reward_profile / staging.
#
# Usage:
#   python scripts/reinforcement_learning/rsl_rl/print_g1_standing_ik_v2_progress_log.py <log_dir>
#
# Options:
#   --last N, --no-plot, --output NAME, --plot-only, --plot-title
...
DEFAULT_V2_PLOT_TITLE = "G1 Standing IK V2 — training progress"
...
# Episode_Reward/* & Episode_Termination/*: missing lines → NaN after first value (no stale carry).
...
def parse_header_standing_ik_v2_reward_config(path: str) -> dict[str, float | str | None] | None:
    """Parse log header for G1 standing IK V2 (and shared standing IK scalars)."""
    try:
...
def generate_visual_analysis(
    ...
    *,
    ...
    plot_title: str = DEFAULT_V2_PLOT_TITLE,
) -> str | None:
    ...
    reward_cfg = parse_header_standing_ik_v2_reward_config(path)
    if reward_cfg is None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Print training_progress_log.txt for G1 Standing IK V2 (g1_standing_ik_v2) and generate the progress PNG."
    )
    parser.add_argument(
        "log_dir",
        type=str,
        help="Run directory, e.g. logs/rsl_rl/g1_standing_ik_v2_phase1/YYYY-MM-DD_HH-MM-SS",
    )
