#!/usr/bin/env python3
"""Training-progress analysis for the Rover OA-Flat task.

Task: Isaac-Rover-OAFlat-v0
...
* ``training_progress_analysis.png`` — six themed panels:
  ...
  Plus a config sidebar parsed from the **Rover OA-Flat** snapshot block in ``train.py``
  headers (camera, depth bins, privileged K, OA reward weights, reverse-vel knobs, curriculum).

Usage
-----
  python scripts/reinforcement_learning/rsl_rl/print_rover_oa_flat_progress_log.py <log_dir>
  python scripts/reinforcement_learning/rsl_rl/print_rover_oa_flat_progress_log.py \
      logs/rsl_rl/rover_oa_flat_env/2026-05-04_12-08-13
...
def parse_header_config(log_path: str) -> dict[str, str]:
    ...
    keys = [
        ...
        "ppo_gamma",
        "ppo_lam",
        # --- Rover OA-Flat header snapshot (train.py) ---
...
def generate_plot(
    ...
    title = (
        f"Rover OA-Flat (depth scan + asymmetric critic + aux) — Progress  "
    ...
    ax4.set_title(
        "Regularisers / blue OA shaping / reverse-vel penalty / tilt / boundary / bonuses",
        fontsize=10, loc="left",
    )
    ...
    sidebar_lines = [
        ...
        "",
        "== OA blue shaping (scaled by oa_obs_scale) ==",
        ...
        f"  yaw_no_clos  : {_v('flat_yaw_no_closure_penalty_weight')}  (penalty wt)",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyse Rover OA-Flat training progress from training_progress_log.txt.",
    )
