...
Class mapping (matches generate_training_data.py):
  duck  → 0
  swan  → 1
  other → 2  (rock, log, lilypad, turtle)
...
YOLO_CLASSES = {'duck': 0, 'swan': 1}   # everything else → 2 (other)
DISTRACTOR_LABELS = {'rock', 'log', 'lilypad', 'turtle'}
    ...
    for bbox_path in sorted(mode_dir.glob("bbox_*.npy")):
        ...
        for box in boxes:
            ...
            if label in YOLO_CLASSES:
                if occ > max_occ:      # skip heavily occluded targets
                    continue
    ...
    for mode_name, ann_name in MODES:
        ...
        for _, lines in records:
            for line in lines:
                ...
                label_counter[["duck", "swan", "other"][cid]] += 1
