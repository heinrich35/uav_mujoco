...
def check_semantic_map(ann: dict) -> list[str]:
    ...
    for sid, info in sorted(smap.items()):
        ...
        if label not in TARGET_LABELS and label != "water" and cls != "other":
...
def draw_visual_check(mode_dir: Path, ann: dict, out_dir: Path, n: int = 9):
    ...
    for bf in bbox_files:
        tag      = bf.stem.replace("bbox_", "")
        rgb_path = mode_dir / f"rgb_{tag}.png"
...
def check_distribution(label_counts: Counter, mode_name: str = "") -> list[str]:
    """Ensure 'other' >= duck+swan, and duck/swan within 2:1.
    ...
    other = sum(v for k, v in label_counts.items()
                if k not in TARGET_LABELS and k != "water")
    ...
    if is_neg:
        ...
        # No duck/swan in negatives is correct — no warning needed
    else:
        if duck == 0:
        ...
        if other < (duck + swan):
            issues.append(f"'other' count ({other}) < duck+swan ({duck + swan}) — "
