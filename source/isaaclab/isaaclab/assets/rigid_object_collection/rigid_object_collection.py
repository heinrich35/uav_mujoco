...
class RigidObjectCollection(AssetBase):
    ...
    def reset(self, env_ids: torch.Tensor | None = None, object_ids: slice | torch.Tensor | None = None):
        ...
        if isinstance(env_ids, torch.Tensor) and env_ids.numel() > 0:
            n_inst = self._external_force_b.shape[0]
            if int(env_ids.max()) >= n_inst:
                raise RuntimeError(
                    "RigidObjectCollection.reset: env_ids exceed internal buffers (would CUDA-index OOB). "
                    f"max(env_ids)={int(env_ids.max())}, num_instances={n_inst}, "
                    f"root_physx_view.count={self.root_physx_view.count}, num_objects={self.num_objects}. "
                    "Typical cause: fabric/replicated cloning left only one env visible to PhysX for this collection "
                    "(try InteractiveSceneCfg.clone_in_fabric=False; or check {ENV_REGEX_NS} prims per env)."
                )
