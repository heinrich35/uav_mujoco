...
class SensorBase(ABC):
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        if isinstance(env_ids, torch.Tensor) and env_ids.numel() > 0:
            n_s = int(self._timestamp.shape[0])
            if int(env_ids.max()) >= n_s:
                raise RuntimeError(
                    "SensorBase.reset: env_ids exceed this sensor's instance count (CUDA index OOB in next line). "
                    f"max(env_ids)={int(env_ids.max())}, num sensor instances={n_s}, prim_path='{self.cfg.prim_path}'."
                )
