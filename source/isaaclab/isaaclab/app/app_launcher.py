...
def _isaaclab_env_truthy(raw: str | None) -> bool:
    """True for common affirmative environment variable spellings."""
    if raw is None:
        return False
    return raw.strip().lower() in ("1", "true", "yes", "on")


def _maybe_warn_r595_driver_for_headless_vulkan_rendering(experience_path: str) -> None:
    """Log when a known-problematic driver branch is paired with Vulkan headless camera rendering."""
    if _isaaclab_env_truthy(os.environ.get("SKIP_ISAACLAB_R595_DRIVER_WARNING")):
        return
    if os.path.basename(experience_path) != "isaaclab.python.headless.rendering.kit":
        return
    try:
        ...
        return
    ver = out.strip().split("\n")[0].strip()
    if not ver.startswith("595."):
        return
    logger.warning(
        "[AppLauncher] NVIDIA driver %s: Isaac Sim 5.1 has reported Hydra RTX / librtx.scenedb crashes on"
        " some 595.xx builds (matching your failure mode). See"
        " https://github.com/isaac-sim/IsaacSim/issues/602 and Isaac Sim 5.1 system requirements for"
        " validated drivers. Blackwell GPUs may require a newer fixed driver branch rather than rolling"
        " back to older Ada-validated builds. Suppress this with SKIP_ISAACLAB_R595_DRIVER_WARNING=1.",
        ver,
    )
    ...
    def _resolve_experience_file(self, launcher_args: dict):
        ...
        if self._sim_experience_file == "":
            ...
            if self._enable_cameras and not self._xr:
                if self._headless and not self._livestream:
                    rendering_kit = "isaaclab.python.headless.rendering.kit"
                    if _isaaclab_env_truthy(os.environ.get("ISAACLAB_DISABLE_VULKAN")):
                        gl_kit_path = os.path.join(
                            isaaclab_app_exp_path, "isaaclab.python.headless.rendering.gl.kit"
                        )
                        if os.path.isfile(gl_kit_path):
                            rendering_kit = "isaaclab.python.headless.rendering.gl.kit"
                            print(
                                "[INFO][AppLauncher]: ISAACLAB_DISABLE_VULKAN is set;"
                                " using OpenGL experience for headless camera rendering."
                            )
                            print(
                                "[WARN][AppLauncher]: If Kit logs"
                                " `[omni.gpu_foundation_factory.plugin] default graphics plugin cannot be set`"
                                " or still crashes in librtx.scenedb, unset ISAACLAB_DISABLE_VULKAN and use an"
                                " NVIDIA driver branch validated for Isaac Sim 5.1 on your GPU (e.g. not 595.xx on"
                                " some Blackwell cards)."
                            )
                        else:
                            print(
                                "[WARN][AppLauncher]: ISAACLAB_DISABLE_VULKAN is set but"
                                " 'isaaclab.python.headless.rendering.gl.kit' was not found;"
                                " falling back to Vulkan headless rendering kit."
                            )
                    self._sim_experience_file = os.path.join(isaaclab_app_exp_path, rendering_kit)
                else:
        ...
        _maybe_warn_r595_driver_for_headless_vulkan_rendering(self._sim_experience_file)
