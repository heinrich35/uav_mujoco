...
def get_checkpoint_path(
    ...
    Args:
        ...
        checkpoint: Regex for the checkpoint filename, or a path to a ``.pt`` file. If this is an
            existing file path, that file is returned. If it contains directories, only the final
            name is used for matching (e.g. ``.../checkpoints/model_2000.pt`` → ``model_2000.pt``).
            Defaults to the most recent torch checkpoint in :attr:`run_dir`.
    ...
    """
    # ``--checkpoint`` is often a full path from shell scripts. Matching is done against
    # ``os.listdir(run_path)`` basenames only, so a path string never matches; also accept
    # an existing file directly.
    if checkpoint:
        if os.path.isfile(checkpoint):
            return os.path.abspath(checkpoint)
        _base = os.path.basename(os.path.normpath(checkpoint))
        if _base and _base != checkpoint:
            checkpoint = _base
